...
protected:
    static inline intptr_t  align(intptr_t n) { return (n + (Alignment - 1)) & ~(Alignment - 1); }
    static inline uintptr_t align(uintptr_t n) { return (n + (Alignment - 1)) & ~(Alignment - 1); }
...
public:
    ...
    char* allocate(size_t n)
    {
        ...
        {  // find slot
            auto it = std::find_if(std::begin(m_freed), std::end(m_freed),
                                   [alignedSize](const std::pair<char*, size_t>& data) {
            ...
            if (it != std::end(m_freed))
            {
                ...
                if (it->second == 0)
                {
                    auto lastIt = std::begin(m_freed) + (m_freed.size() - 1);
...
public:
    ...
    ~ArenaAllocator()
    {
        const size_t freed = std::accumulate(
            std::begin(m_freed), std::end(m_freed), 0,
            [](size_t accum, const std::pair<T*, size_t>& data) { return accum + data.second; });
    ...
    {
        m_freed.emplace_back(p, n);
        // std::sort(std::begin(m_freed), std::end(m_freed), [](const std::pair<T*, size_t>& d1,
        // const std::pair<T*, size_t>& d2) {
        //     return d1.second < d2.second;
        // });
    }

    void construct(T* p, const T& v) { p = new (p) T(v); }
    void destroy(T* p) { p->~T(); }
};

template <typename T, typename TPointer = std::unique_ptr<T>,
...
public:
    ...
    static_assert(std::is_same<T*, TPointer>::value == false,
                  "Pointer type must be some kind of smart pointer");

public:
    Factory(Allocator& allocator) : m_allocator(allocator) {}
